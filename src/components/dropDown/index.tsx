...
export type ASDropDownProps = Omit<DropdownProps<any>,
...
const ASDropDown: React.FC<ASDropDownProps> = (props: ASDropDownProps) => {
    ...
    const renderSingleChoiceItem = (item: DropDownOptionsProps) => {
        ...
        return (
            <View style={[styles.item,{...(isSelected ? {backgroundColor: hexToRgbaWithOpacity(colors.primary)} : {})}]}>
                <Text
                    style={[
                        styles.textItem,
                        {
    ...
    const renderMultipleChoiceItem = (item: DropDownOptionsProps) => {
        const isSelected = Array.isArray(field?.value) && field?.value?.some((_value: string | number) => _value === item?.value)
        return (
            <View
                style={[styles.item, {...(isSelected ? {backgroundColor: hexToRgbaWithOpacity(colors.primary)} : {})}]}>
                <Text
                    style={[
                        styles.textItem,
                        {
    ...
    const renderSelectedItem = (item: any, unSelect?: (_item: any) => void) => {
        return (
            <ASButton
                style={[styles.multipleSelectionButton, {borderColor: colors.primary}]}
        ...
        <View
            style={[
                ...
                {  /* Need to have this logic beacuse the DropDown has extra padding so it always a bit bigger to other widget even though they have the same padding
                    ...
                    */
                    paddingTop: typeof flatttenedContainerStyle?.paddingTop === 'number' && flatttenedContainerStyle.paddingTop > 0 ? flatttenedContainerStyle.paddingTop - 1 : 0,
                    paddingBottom: typeof flatttenedContainerStyle?.paddingBottom === 'number' && flatttenedContainerStyle.paddingBottom > 0 ? flatttenedContainerStyle.paddingBottom - 1 : 0,
                }
            ]}
            id={id}
        >
